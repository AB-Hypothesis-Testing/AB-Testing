...
class LogisticRegression:
    ...
    def __init__(X_train, X_test, y_train, y_test, max_depth=5):
        ...
        clf = LogisticRegression(max_depth=4)
    ...
    def train(self, folds=1):
        ...
        for i in range(folds):
            ...
            self.X_train, y_train = self.X_train.iloc[train_index], self.y_train.iloc[train_index]
            self.X_valid, y_valid = self.X_train.iloc[valid_index], self.y_train.iloc[valid_index]
                        
            self.clf = self.clf.fit(self.X_train, self.y_train)
            
            vali_pred = self.clf.predict(self.X_valid)
            
            accuracy = self.calculate_score(self.y_valid
                                              , vali_pred)
    ...
    def __printAccuracy(self, acc, step=1, label=""):
        print(f"step {step}: {label} Accuracy of LogisticRegression is: {acc:.3f}")
    
    def __printLoss(self, loss, step=1, label=""):
        print(f"step {step}: {label} Loss of LogisticRegression is: {loss:.3f}")
